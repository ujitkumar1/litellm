...
The same goes for filing an issue: treat every field's `description` and `placeholder` in @.github/ISSUE_TEMPLATE/bug_report.yml and @.github/ISSUE_TEMPLATE/feature_request.yml as rules to follow, not just layout, and read the one you're filing from disk before writing the issue body, since the rendered form and any copy injected into your context can drop or reflow that guidance
