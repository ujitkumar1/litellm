...
class ResetBudgetJob:
    ...
    async def _reset_windows_for(
        ...
        """Walk one table's windowed rows a page at a time.

        The scan is keyset-paginated rather than read whole: the unbounded form
        materialized every row carrying budget_limits on every tick, which grows
        ...
        verbose_proxy_logger.warning(
            "Reset budget job: stopped scanning %s budget windows after %s pages of %s; "
            "the remaining rows are swept on the next tick",
            source.log_subject,
            RESET_BUDGET_JOB_MAX_CHUNKS_PER_RUN,
            RESET_BUDGET_JOB_BATCH_SIZE,
        )
