...
def test_reset_budget_windows_stops_at_the_per_run_cap(monkeypatch):
    """A table that never returns a short page must not spin for the whole tick.
    The cap bounds the run, and the operator is told the sweep was truncated
    rather than being left to assume it completed."""
    monkeypatch.setattr(reset_budget_job_module, "RESET_BUDGET_JOB_BATCH_SIZE", 1)
    monkeypatch.setattr(reset_budget_job_module, "RESET_BUDGET_JOB_MAX_CHUNKS_PER_RUN", 3)
    past = datetime.utcnow() - timedelta(hours=2)
    job, calls = _paginating_window_job(
        monkeypatch,
        {
            "key": [[_window_row("token", f"k{i}", past)] for i in range(50)],
            "team": [[]],
        },
    )
    warnings: List[str] = []
    monkeypatch.setattr(
        reset_budget_job_module.verbose_proxy_logger,
        "warning",
    ...
    assert len([call for call in calls if call["table"] == "key"]) == 3
    assert any("stopped scanning keys budget windows" in warning for warning in warnings)
